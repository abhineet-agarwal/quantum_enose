...
| V points | 51 (16 mV) — the paper grid | clean I-V; raw d2I works. 201-pt is denser but its raw d2I is jitter-dominated |
...
- **Golden npz (all 51-pt, the paper grid):**
  `results/sispad_scba_2026-04-14/iets_ZnO_MgZnO_symmetric_*_0-800mV_*_rank1scba_*.npz`
  - 300 K: Baseline/Mol_A/Mol_B/Mol_AB (`*_2026-04-14.npz`).
  - temp sweep: Mol_A at 10/77/150 K (`*_2026-05-27.npz`, re-run at 51 pts).
  - `make_paper_figs.py` loads the 51-pt files (≤60 V-points).
- **Paper figures:** `results/sispad_scba_2026-04-14/`
  `fig1_IV`, `fig2_d2IdV2`, `fig3_deltaI`, `fig4_deltaD`, `fig5_temp`
  (+ schematics `fig2_band_diagram`, `fig3_flowchart`, `fig1_table_stacks`),
  generated by `run/make_paper_figs.py`. The Apr-14 PNG originals and the broken
  Apr-28 set are preserved under
  `archive/results/sispad_scba_2026-04-14_nonpaper/{_correct_apr14_backup,_published_apr28_backup}/`.
- **Overleaf rename mapping** (the paper `.tex` was edited on Overleaf):
  local `fig1_IV` → `fig4_IV`, `fig2_d2IdV2` → `fig5_d2IdV2`,
  `fig4_deltaD` → `fig7_deltaD`, `fig5_temp` → `temp.png`;
  band diagram/flowchart kept their names.
...
**Temperature figure (`fig5_temp`, paper Fig 4):** regenerated by
`make_paper_figs.py` as a consistent 2-panel figure — Mol_A at 10/77/150/300 K,
all on the 51-pt grid with the SAME raw `np.diff(I,2)/dV²` as Fig 3 (the temp
sweeps at 10/77/150 K were re-run at 51 pts on 2026-05-27). This intentionally
does NOT reproduce the published Fig 4b, which used the analytic d2I field
(±750 µA/V² with a clean 10K≫300K ordering); the consistent numerical version is
±300 µA/V² with a milder ordering. The paper Fig 4 should be updated to match.
...
- `pytest tests/` → 47 pass, **4 pre-existing failures** in
  ...
  open tightening item in `archive/docs/SISPAD_CHECKLIST.md`, not the ZnO production case and
