...
Dnu=[0.1,0.1];
Nph=size(Dnu,2);
hnu=[18,35];%Multiply by dE for actual hnu
...
%save('rtd_1d_flat.mat')
save(['rtd_1d_flat' num2str(hnu(1)*dE) 'eV_' num2str(hnu(2)*dE) 'eV0.mat'])
%save(['rtd_1d_flat' num2str(hnu(1)*dE) 'eV.mat'])
...
grid on
hold on
h=plot(VETS,diff(G2)./G2(2:end));
ylabel('$(d^2I/dV^2)/(dI/dV)$ $[V^{-1}]$')
xlabel('Bias Voltage [V]')
% saveas(figure(1),['IETS_rtd_np_NOfSites' num2str(Np) '.png'])
% saveas(figure(1),['IETS_rtd_np_NOfSites' num2str(Np) '.fig'])
% saveas(figure(1),['IETS_rtd_np_NOfSites' num2str(Np) '.eps'], 'epsc2')
